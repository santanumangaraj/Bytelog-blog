...
## Appendix: AWS EC2 self-host (Docker Compose)

Same idea as the Oracle appendix above, but on AWS specifically — useful if
you're spending AWS credit and want hands-on EC2/Docker experience rather
than Render's managed runtime. **MySQL stays on Aiven and Redis stays on
Upstash** (no reason to also self-host those here, and no domain is
required — [sslip.io](https://sslip.io) gives Caddy a real hostname to
provision HTTPS for, for free, based on your instance's IP).

Compute only: `docker-compose.prod.yml` (new, alongside the existing
`docker-compose.yml`) runs just `backend` + `blog-worker` + `like-worker` +
`caddy` — no local `mysql`/`redis` services, which matters because the dev
compose file's `depends_on: mysql: condition: service_healthy` would
otherwise block every backend process from starting without a real local
MySQL running (the exact issue hit locally earlier in this project).

### 0. Set a spending guardrail first
AWS Console → **Billing** → **Budgets** → create a budget with alert
thresholds (e.g. $20 / $50 / $80) emailing you. Do this before launching
anything — a forgotten running resource is the most common way credit
disappears faster than expected.

### 1. Launch the EC2 instance
- EC2 console → **Launch Instance**
- AMI: **Ubuntu Server 22.04 or 24.04 LTS**
- Instance type: **t3.small** (2 GB RAM — 3 Node processes + Caddy is tight
  on a 1 GB `t3.micro`; t3.small is safer and still cheap)
- Key pair: create new, download the `.pem`, keep it safe
- Security group — create new:
  - SSH (22) — source **My IP** only, never `0.0.0.0/0`
  - HTTP (80) — source **Anywhere** (needed for the Let's Encrypt HTTP challenge)
  - HTTPS (443) — source **Anywhere**
  - **Do not open 8002** — Caddy is the only public entrypoint; the backend
    is reachable only inside the Docker network (see `expose:` vs `ports:`
    in `docker-compose.prod.yml`)
- Storage: default 8–20 GB gp3 is plenty
- Launch

### 2. Allocate an Elastic IP
EC2 console → **Elastic IPs** → Allocate → Associate with your new
instance. This keeps the public IP stable across stop/start — important
both generally and specifically because sslip.io encodes the IP into the
hostname itself.
...
```
git clone https://github.com/<you>/bytelog-blog.git
cd bytelog-blog
```

### 5. Production env files
`backend/.env` — same Aiven/Upstash/AWS S3 values you already use, plus:
```
NODE_ENV=production
TRUST_PROXY=true
IS_DOCKER=true
...
```
Root `.env` (only `DOMAIN` matters here — `docker-compose.prod.yml` doesn't
reference `DB_PASS`/`DB_NAME` since it has no local mysql service):
```
DOMAIN=<elastic-ip-with-dots-as-dashes>.sslip.io
```
e.g. Elastic IP `3.123.45.67` → `DOMAIN=3-123-45-67.sslip.io`
...
```
Watch for: DB connected, Redis connected, server listening, both workers
initialized, and Caddy successfully obtaining a certificate for the
`sslip.io` hostname.

### 7. Migrations (unchanged from the Render path)
```
cd backend
npx sequelize-cli db:migrate
```
...
Update `frontend/vercel.json`'s rewrite destination from the Render URL to
your new `sslip.io` domain:
```json
{ "source": "/api/v2/:path*", "destination": "https://3-123-45-67.sslip.io/api/v2/:path*" }
```
Keep `VITE_API_URL=/api/v2` unchanged on Vercel — the whole point of that
rewrite (see Part 3 above) is keeping the API same-origin from the
browser's perspective so the auth cookie isn't third-party. Changing
`VITE_API_URL` to point directly at the EC2 domain instead of updating this
rewrite would reintroduce that cross-site cookie bug. Redeploy on Vercel
after committing the change (rewrites are read at build/deploy time).

### 9. Verify
```
curl https://3-123-45-67.sslip.io/health
```
Expected: `{"status":"ok","db":"up","redis":"up"}`, no cold start this
time — this instance doesn't sleep.

### Later, if you want to go further with the credit
- Move MySQL to **RDS** and/or Redis to **ElastiCache** — real AWS-managed
  services, but budget for them explicitly (unlike Aiven/Upstash's free
  tiers, these bill continuously from the start).
- Move to **ECS Fargate** instead of a raw EC2 box once comfortable — no
  server to patch, but adds an ALB (~$16/mo idle) and more moving parts.
- Point a **Route 53** hosted zone at the Elastic IP instead of sslip.io,
  once/if you buy a domain.
